...
const createShortenURL = async function (req, res) {
    try {
        ...
        if (validURL.isWebUri(longUrl.toString())) {
            ...
            let cachedlinkdata = await GET_ASYNC(`${longUrl}`)
            ...
            if (cachedlinkdata) {
                ...
                const findUrl = await urlModel.findOne({ longUrl }).select({ _id: 0, createdAt: 0, updatedAt: 0, __v: 0, })
                if (findUrl) {
                    await SET_ASYNC(`${longUrl}`, JSON.stringify(findUrl), "EX", 60 * 2)
                    return res.status(200).send({ status: true, msg: "data coming from db", data: findUrl })
                }
                else {
